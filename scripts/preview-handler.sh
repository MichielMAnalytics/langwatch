#!/usr/bin/env bash
# Handler for the `/boxd-preview` slash-command on a PR. Invoked by webhook(8)
...
#   $4  comment.id    (for reacting to the triggering comment)
#
...
if [ -t 1 ] || [ "${PREVIEW_FOREGROUND:-0}" = "1" ]; then
  : # already in fg (manual run or re-execed self)
else
...
echo
echo "$(date -u +%FT%TZ) /boxd-preview from @$COMMENTER on $REPO#$PR_NUMBER (comment $COMMENT_ID)"

# Acknowledge immediately so the user sees we picked up the command.
# Layer-A already filtered out randoms, so reacting here is safe.
...
case "$PERM" in
  ...
  *)
    gh pr comment "$PR_NUMBER" --repo "$REPO" --body \
      "@$COMMENTER /boxd-preview requires write access to this repo. Your effective permission is \`$PERM\`."
...
gh pr comment "$PR_NUMBER" --repo "$REPO" --body \
  "⏳ booting a boxd preview env for @$COMMENTER (perm: \`$PERM\`)…"
echo "  acknowledged; fork-and-deploy not yet wired up"
